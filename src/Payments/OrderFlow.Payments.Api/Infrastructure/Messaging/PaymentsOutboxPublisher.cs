...
public sealed class PaymentsOutboxPublisher : BackgroundService
{
    ...
    private readonly ILogger<PaymentsOutboxPublisher> logger;
    private readonly string owner = $"{Environment.MachineName}-{Guid.NewGuid():N}";

    public PaymentsOutboxPublisher(IServiceScopeFactory scopeFactory, PulsarEventBus eventBus, ILogger<PaymentsOutboxPublisher> logger)
    {
    ...
    private async Task<bool> PublishOneAsync(CancellationToken cancellationToken)
    {
        ...
        var message = await db.OutboxMessages.FromSqlInterpolated($"""
            SELECT * FROM "orderflow_payments"."outbox_messages"
