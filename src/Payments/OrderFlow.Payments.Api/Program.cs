...
using OrderFlow.Payments.Api;
using OrderFlow.Payments.Infrastructure.Database;
...
builder.Services.AddDbContext<PaymentsDbContext>((serviceProvider, options) =>
{
    ...
    options.UseNpgsql(database.ConnectionString, npgsql =>
        npgsql.MigrationsHistoryTable("__EFMigrationsHistory", "orderflow_payments"));
});
...
app.MapGet("/", () => Results.Ok(new { service = "payments" }));
