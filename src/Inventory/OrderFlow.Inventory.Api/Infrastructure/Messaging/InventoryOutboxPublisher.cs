...
public sealed class InventoryOutboxPublisher : BackgroundService
{
    ...
    private readonly ILogger<InventoryOutboxPublisher> logger;
    private readonly string owner = $"{Environment.MachineName}-{Guid.NewGuid():N}";

    public InventoryOutboxPublisher(IServiceScopeFactory scopeFactory, PulsarEventBus eventBus, ILogger<InventoryOutboxPublisher> logger)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                if (!await PublishOneAsync(stoppingToken)) await Task.Delay(PollInterval, stoppingToken);
            }
    ...
    private async Task<bool> PublishOneAsync(CancellationToken cancellationToken)
    {
        ...
        var message = await db.OutboxMessages.FromSqlInterpolated($"""
            SELECT * FROM "orderflow_inventory"."outbox_messages"
