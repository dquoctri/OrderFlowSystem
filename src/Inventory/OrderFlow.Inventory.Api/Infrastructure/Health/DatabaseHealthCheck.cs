...
public sealed class DatabaseHealthCheck : IHealthCheck
{
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            await using var command = new NpgsqlCommand("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'orderflow_inventory' AND table_name = '__EFMigrationsHistory')", connection);
            var migrated = (bool)(await command.ExecuteScalarAsync(cancellationToken) ?? false);
            return migrated ? HealthCheckResult.Healthy() : HealthCheckResult.Unhealthy("Inventory database migrations have not been applied.");
        }
