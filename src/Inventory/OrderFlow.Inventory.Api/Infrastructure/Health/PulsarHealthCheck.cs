...
public sealed class PulsarHealthCheck : IHealthCheck
{
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(adminUrl)) return HealthCheckResult.Unhealthy("Pulsar admin URL is not configured.");
        try
        {
            ...
            return response.IsSuccessStatusCode ? HealthCheckResult.Healthy() : HealthCheckResult.Unhealthy($"Pulsar returned {(int)response.StatusCode}.");
        }
        catch (Exception exception) { return HealthCheckResult.Unhealthy(exception.Message); }
    }
}
