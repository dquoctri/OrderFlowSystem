...
database per service, and a Blazor WebAssembly client.
...
| Client | Blazor WebAssembly (static SPA behind nginx) |
...
| Patterns | Transactional outbox, inbox dedup, `SELECT … FOR UPDATE` stock locking, choreographed saga (no orchestrator, no saga framework) |
| Runtime | Docker Compose — one-shot migrator containers, single `edge` proxy owning host ports 5000–5003 |
...
│   └── Web/OrderFlow.Web/
│       ├── Api/                     # typed OrderFlowApiClient + view models
...
`*-db` → `*-migrator` (applies EF Core migrations once) → `*-api` (healthy) → `web` + `edge`.
...
| `GET localhost:5001/orders/{id}` | order status + `failureStage` / `failureReason` / `completedAt` |
| `GET localhost:5001/orders/{id}/trace` | the ordered saga event log for one order |
| <http://localhost:5001/health/ready> | Orders — `200` only when its DB + Pulsar are reachable |
| <http://localhost:5002/health/ready> · <http://localhost:5003/health/ready> | Inventory · Payments |
...
## Scale (stateless API replicas)

```bash
docker compose up --build --scale orders-api=3 --scale inventory-api=3 --scale payments-api=3
```

Only `edge` publishes host ports; API replicas expose only their internal `8080`, and nginx
resolves the scaled names via Docker DNS — so replicas never collide. Migrations still run once
(in the migrator containers), and the outbox relay claims rows with `FOR UPDATE SKIP LOCKED`, so
multiple replicas are safe. If you change replica counts while running, recreate `edge` so it
re-resolves upstreams: `docker compose up -d --force-recreate edge`.

## Configuration & secrets

- Backend config is **environment variables only** (`Database__ConnectionString`,
  `Pulsar__ServiceUrl`, `Pulsar__AdminUrl`), set in `docker-compose.yml`. Names are fixed.
  Options are validated at startup (`ValidateOnStart`) — a bad connection string or Pulsar URL
  fails the boot with a readable message rather than at first use.
...
- The Blazor client is a static SPA and reads API base URLs from `wwwroot/appsettings.json`
  (`ApiUrls:Orders`, `ApiUrls:Inventory`). No secrets or connection strings in the client.
...
| Suite | Needs | Proves |
| --- | --- | --- |
| **Unit** — `Orders.Tests`, `Payments.Tests`, most of `Inventory.Tests` | nothing | `OrderValidation` (request rules + total), `OrderSagaTransitions` (every status move + guard), `StockOperations` (reserve/release/consume + invariants), `OrderReservation` (all-or-nothing), `FakePaymentGateway` via `IPaymentGateway` (the `.99` rule), `PaymentRecord` (charge → event), `PoisonMessagePolicy` (DLQ after 3), `RedeliveryPolicy` (2/5/10s backoff) |
